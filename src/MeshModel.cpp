...
size_t MeshModel::getMeshCount() { return meshList.size(); }
...
std::vector<std::string> MeshModel::LoadMaterials(const aiScene *scene) {
  ...
  for (size_t i = 0; i < scene->mNumMaterials; i++) {
    ...
    // Initialise the texture to empty string (will be replaced if texture
    // exists)
    textureList[i] = "";
...
std::vector<Mesh> MeshModel::LoadNode(VkPhysicalDevice newPhysicalDevice,
  ...
  // Go through each mesh at this node and create it, then add it to our
  // meshList
  for (size_t i = 0; i < node->mNumMeshes; i++) {
    meshList.push_back(LoadMesh(
	newPhysicalDevice, newDevice, transferQueue, transferCommandPool,
	scene->mMeshes[node->mMeshes[i]], scene, matToTex));
  }

  // Go through each node attached to this node and load it, then append their
  // meshes to this node's mesh list
